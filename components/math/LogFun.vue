...
function logFun() {
  functionPlot({
    ...
    tip: {
      renderer(x, y, index) {
        const formula = index === 0 ? `y = log(a)/log(x) (a > 1)` : `y = log(a)/log(x) (0 < a < 1)`
        return `(${formatTooltip(x)}, ${formatTooltip(y)}) ${formula}`
      },
    },
    data: [
      {
        fn: `log(x)/log(${incrementalFunVal.value})`,
        graphType: 'polyline',
        color: '#F56C6C',
        range: [0.01, Infinity],
      },
      {
        fn: `log(x)/log(${decreaseFunVal.value})`,
        graphType: 'polyline',
        color: '#67C23A',
        range: [0.01, Infinity],
      },
